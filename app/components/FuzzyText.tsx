    ...
    const init = async () => {
      ...
      const run = (timestamp: number) => {
        ...
        if (transitionDuration > 0) {
          ...
          if (currentIntensity < target) currentIntensity = Math.min(currentIntensity + step, target);
          else if (currentIntensity > target) currentIntensity = Math.max(currentIntensity - step, target);
        } else {
