...
export interface ProfileCardProps {
  avatarUrl?: string;
  /** When set, shows a looping muted video instead of avatar images (lighter with preload="metadata" + poster). */
  avatarVideoUrl?: string;
  /** Shown before the video loads; defaults to avatarUrl. */
...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  const useVideoAvatar = Boolean(avatarVideoUrl);
  const posterForVideo = avatarPosterUrl ?? avatarUrl;
  ...
  useEffect(() => {
    if (!hoverAvatarUrl || useVideoAvatar) return;
    const img = new Image();
    img.src = hoverAvatarUrl;
  }, [hoverAvatarUrl, useVideoAvatar]);
  ...
  useEffect(() => {
    ...
    let pauseDebounce: number | null = null;
    ...
    /** Debounce: klik/hover kadang memicu pause singkat; resume langsung memicu tarikan decode → tersendat. */
    const onPause = (): void => {
      if (pauseDebounce != null) window.clearTimeout(pauseDebounce);
      pauseDebounce = window.setTimeout(() => {
        pauseDebounce = null;
  ...
  const avatarMediaBaseStyle: React.CSSProperties = {
    width: "100%",
    position: "absolute",
    left: "50%",
    bottom: "-1px",
  ...
  /** Video: tanpa parallax/transition pada elemen video — mengurangi repaint & jeda decode saat kursor/klik. */
  const avatarVideoStyle: React.CSSProperties = {
    ...avatarMediaBaseStyle,
